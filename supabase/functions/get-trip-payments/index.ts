...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      console.error("get-trip-payments query error:", error);
      return json({ code: "internal_error", message: "Failed to fetch payments" }, 500);
    }

    return json({ payments: payments ?? [] });
  } catch (err) {
    console.error("get-trip-payments error:", err);
